...
export default function ArticleCard({ article, isRead, onRead, now }) {
  ...
  // CRAAP shown only as a tooltip — credibility signal without visual clutter
  const craapTitle = article.craap_score != null
    ? `Source quality: ${article.craap_score}/25 — Currency · Relevance · Authority · Accuracy · Purpose`
    : undefined;

  return (
    <div className={`border border-theme rounded-lg overflow-hidden
      transition-all duration-150 bg-card hover:shadow-card`}
    >
      <div className="flex">
        {/* Read stripe */}
        ...
        <div className="flex-1 px-5 py-4 min-w-0">
          ...
          {/* Meta row — source · time on left, importance label on right */}
          <div className="flex items-center justify-between mb-2.5">
            ...
            {/* Importance label — right-aligned, only Breaking/Critical/Important */}
            <ImportanceBadge level={article.importance_level} />
          </div>

          {/* Headline — serif for editorial credibility */}
          <p className="font-serif text-[15px] font-bold leading-snug text-primary mb-0">
            {article.title}
          </p>
